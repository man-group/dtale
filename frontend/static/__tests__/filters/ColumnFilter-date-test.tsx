...
describe('ColumnFilter date tests', () => {
  ...
  it('ColumnFilter date rendering', async () => {
    expect(result.getElementsByClassName('bp6-input').length).toBeGreaterThan(0);
    await act(async () => {
      await fireEvent.click(screen.getByText('Missing'));
    });
    expect(result.getElementsByClassName('bp6-disabled')).toHaveLength(2);
    await act(async () => {
      await fireEvent.click(screen.getByText('Missing'));
    });
    expect(result.getElementsByClassName('bp5-disabled')).toHaveLength(0);
    await act(async () => {
      await fireEvent.change(result.getElementsByClassName('bp6-input')[0], { target: { value: '20000102' } });
    });
    expect(spies.saveSpy).toHaveBeenLastCalledWith('1', 'col4', { type: 'date', start: '20000102', end: '20000131' });
    await act(async () => {
      await fireEvent.change(result.getElementsByClassName('bp6-input')[1], { target: { value: '20000103' } });
    });
