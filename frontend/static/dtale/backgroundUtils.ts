...
import { ColumnDef, DataRecord, DataViewerState, OutlierRange, RangeHighlightModes } from './DataViewerState';
...
const heatMapBackground = (record: DataRecord, range: ColumnDef): React.CSSProperties => {
  if (record.view === '') {
    return {};
  }
  const { min, max } = range;
...
export const updateBackgroundStyles = (
  ...
  switch (state.backgroundMode) {
    ...
    case 'heatmap-all-all': {
      const overall = { ...state, ...state.filteredRanges?.overall } as ColumnDef;
