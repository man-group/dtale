...
describe("FilterInput", () => {
  ...
  it("handle input-style filters", () => {
    ...
    expect(props.save).toHaveBeenCalledWith(inputFilter.name, 2);
  ...
  it("handle input-style float filter", () => {
    ...
    wrapper.find("button").last().simulate("click");
    expect(props.save).toHaveBeenCalledWith(inputFilter.name, 1.1);
    expect(fetchJsonSpy).not.toHaveBeenCalled();
  });
  ...
  it("handle input-style string filter", () => {
    ...
    wrapper.find("input").simulate("change", { target: { value: "a" } });
    wrapper.find("button").last().simulate("click");
    expect(props.save).toHaveBeenCalledWith(inputFilter.name, "a");
    expect(fetchJsonSpy).not.toHaveBeenCalled();
  });
  ...
  it("handle select-style filters", () => {
    ...
    wrapper.setProps({ filter: selectFilter, value: 1 });
    wrapper.find("button").simulate("click");
    ...
    wrapper.find("button").last().simulate("click");
    expect(props.save).toHaveBeenCalledWith(selectFilter.name, 2);
  ...
  it("handle multiselect-style filters", () => {
    ...
    wrapper.setProps({ filter: multiselectFilter, value: [1, 2] });
    wrapper.find("button").simulate("click");
    expect(wrapper.find(ValueSelect)).toHaveLength(1);
    wrapper
      .find(ValueSelect)
    ...
    wrapper.find("button").last().simulate("click");
    expect(props.save).toHaveBeenCalledWith(multiselectFilter.name, [2, 3]);
    expect(fetchJsonSpy).toHaveBeenCalledTimes(1);
  });
});
