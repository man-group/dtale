...
def upload():
    ...
    for filename in request.files:
        ...
        if ext in [".csv", ".tsv"]:
            ...
            df = pd.read_csv(contents, engine="python", sep=None)
            return load_new_data(df, "df = pd.read_csv('{}', engine='python', sep=None)".format(filename))
        if ext in [".xls", ".xlsx"]:
            ...
            for sheet_name in sheet_names:
                df = dfs[sheet_name]
                code = "df = pd.read_excel('{}', sheet_name='{}', engine='{}')".format(filename, sheet_name, engine)
