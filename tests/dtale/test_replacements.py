...
@pytest.mark.skipif(parse_version(skl.__version__) < parse_version('0.21.0'), reason="requires scikit-learn 0.21.0")
...
@pytest.mark.skipif(parse_version(skl.__version__) < parse_version('0.22.0'), reason="requires scikit-learn 0.22.0")
...
@pytest.mark.skipif(parse_version(skl.__version__) < parse_version('0.20.0'), reason="requires scikit-learn 0.20.0")
...
def test_view(unittest):
    ...
    with app.test_client() as c:
        ...
        with ExitStack() as stack:
            ...
            assert response_data['error'] == "'not_implemented' replacement not implemented yet!"

            params = dict(
                type='value',
                col='e',
                name='a',
                cfg=json.dumps({'value': [dict(value='nan', type='raw', replace='for test')]})
            )
